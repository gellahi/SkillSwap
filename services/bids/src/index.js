...
// Mount API routes
app.use('/api/bids', routes);

// Root route
app.get('/', (req, res) => {
  res.json({
    message: 'Welcome to SkillSwap Bids Service',
    version: '1.0.0',
    endpoints: [
      '/api/bids',
      ...
      '/api/bids/:id',
      '/api/bids/:id/counter-offer',
      '/api/bids/:id/milestones',
      '/api/bids/:id/feedback'
    ]
  });
});
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      logger.info(`Bids service running in ${process.env.NODE_ENV} mode on port ${PORT}`);
    });
  } catch (error) {
