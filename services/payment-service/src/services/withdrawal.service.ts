  ...
  async processWithdrawal(id: string): Promise<Withdrawal> {
    try {
      ...
      try {
        ...
        } else if (withdrawal.paymentMethod.type === PaymentMethodType.PAYPAL) {
          // Process PayPal payout
          const payoutData = await this.paypalService.createPayout(
            withdrawal.paymentMethod.details.email,
            withdrawal.netAmount,
            'USD',
            `Withdrawal from SkillSwap - ID: ${withdrawal.id}`,
          );
          
          externalReference = payoutData.batch_header.payout_batch_id;
        } else {
